...
class DQN(object):
    ...
    def choose_action(self, x):
        ...
        x = x.permute(
            0, 3, 2, 1
        )  # 把图片维度从[batch, height, width, channel] 转为[batch, channel, height, width]
    ...
    def push_memory(self, obs, a, r, obs_):
        ...
        self.memory[self.position] = Transition(
            torch.unsqueeze(torch.FloatTensor(obs), 0),
            torch.unsqueeze(torch.FloatTensor(obs_), 0),
...
def main():
    ...
    if train:
        ...
        # =============== test ================
        dqn.eval_net.load_state_dict(torch.load("EvalNet-5.pt"))
        dqn.eval_net.eval()
        dqn.target_net.load_state_dict(torch.load("TargetNet-5.pt"))
        dqn.target_net.eval()
        for episode in range(100):
            obs = env.reset()
            for i in range(1000):
